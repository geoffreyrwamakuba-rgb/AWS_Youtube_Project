...
SNS_TOPIC = os.environ.get("SNS_ALERT_TOPIC_ARN", "arn:aws:sns:eu-north-1:225619512731:yt-data-pipeline-alerts-dev")
S3_BUCKET_SILVER = os.environ.get("S3_BUCKET_SILVER", "youtube-data-gr-silver")
ATHENA_RESULTS_BUCKET = os.environ.get("ATHENA_RESULTS_BUCKET", "youtube-data-gr-lambda-athena-results")
ATHENA_OUTPUT = f"s3://{ATHENA_RESULTS_BUCKET}/query-results/"
logger.info(f"Athena output path: {ATHENA_OUTPUT}")
...
def check_null_percentage(df: pd.DataFrame, table_name: str) -> list:
    ...
    for col in cols:
        if col not in df.columns:
            ...
            continue # skips below and moves to the next col
...
def lambda_handler(event, context):
    ...
    {
        "layer": "silver",
        "database": "youtube-data-gr-silver",
        "tables": ["clean_statistics", "clean_reference_data"]
    }
    """
    database = event.get("database", "youtube-data-gr-silver")
    ...
    for table_name in tables:
        ...
        try:
            ...
            # sample chosen as trade-off, not an oversight.
            # Reduced Athena Costs
            # Faster - avoide looking millions of rows
            # Satistical sufficiency - 10k enough to see systemic issues but could be caught off by skewed data
    ...
    return {
        ...
        # Lambda return values must be JSON-serialisable 
        # This is a common trick to force all values into JSON-safe types in one shot.
    }